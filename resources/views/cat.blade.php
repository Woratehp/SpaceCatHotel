<title>หน้าแมว | Spacecathotal</title>
...
@section('Content')
        ...
        <div id= "modal-Profile-cat" class="border border-gray-300 relative mt-4 flex items-center py-3">    
                <button class="absolute border border-gray-400 flex w-fit mt-2 mb-2 rounded-lg px-2 py-1 ml-7 ">
                        ...
                        <a href="home" class="ml-1.5">กลับ</a>
                </button>  
             ...
             @foreach($cats as $cat)
                ...
                <div class="flex flex-col items-center pb-10">
                    ...
                    <div class="flex mt-4 md:mt-6">
                        <button data-cat_id="{{ $cat->cat_id }}" data-cat_name="{{ $cat->cat_name }}" data-cat_breed="{{ $cat->cat_breed }}" data-cat_weight="{{ $cat->cat_weight }}"
                        data-cat_gender="{{ $cat->cat_gender }}" data-cat_date="{{ $cat->cat_date }}" data-cat_pic="{{ URL('/uploads/'.$cat->cat_pic) }}"  data-cat_document="{{ URL('/uploads/'.$cat->cat_document) }}" 
                        class="btn-edit-cat inline-flex items-center px-4 py-2 text-sm font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-700 dark:focus:ring-gray-700 ms-3">แก้ไข</button>
        ...
        @endif
            ...
            <div id="modal-add-cat" class="modal hidden fixed z-[100] flex my-auto left-0 top-0 w-[100%] h-[100%] overflow-auto  " >
                <div class="modal-content bg-white m-auto p-[10px] rounded-md drop-shadow-xl w-[65%] max-md:w-[100%] gap-2">
                        ...
                        <div class="col-span-2 relative border 300 w-full bg-white shadow-lg rounded-xl p-3">
                            <div class="grid grid-cols-2 gap-3"> 
                                <div>
                                    ...
                                    <div class="p-2 rounded-xl border flex w-full bg-gray-100 shadow-md">
                                        <input id="cat_name" class="ml-2 font-light bg-gray-100 outline-none"  type="name" name="name" placeholder="กรอกชื่อแมว">   
                                    </div>       
                                </div> 
                                <div>
                                    <label for="email" class="ml-3 ml-2 text-sm font-medium text-gray-900 ">พันธ์แมว :</label>
                                    <div class="p-2 rounded-xl border flex w-full bg-gray-100 shadow-md">
                                        <input id="cat_breed" class="ml-2 font-light bg-gray-100  outline-none"  type="name" name="name" placeholder="กรอกพันธ์แมว">   
                                    </div>       
                                </div>     
                            ...
                            <div class="w-fit h-fit my-auto flex  mt-2"> 
                                <div class="">
                                    ...
                                    <div class="p-2 rounded-xl border flex w-full bg-gray-100 shadow-md">
                                        <input type="number" id="cat_weight" class="ml-2 font-light bg-gray-100  outline-none "  type="name" name="name" placeholder="น้ำหนักแมวแมว">   
                                    </div>       
                                </div>     
                                <div class="ml-[5.5em]">
                    ...
                    <button onClick="SubmitCat()" type="button" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 rounded-lg text-sm px-5 py-2.5 text-center ml-auto mr-5 flex">บันทึก</button>       
            </div>
        </div>
            <!-- start_modal_editcat -->
            <div id="modal-edit-cat" class="modal hidden fixed z-[100] flex my-auto left-0 top-0 w-[100%] h-[100%] overflow-auto  ">
                <div class="modal-content bg-white m-auto p-[10px] rounded-md drop-shadow-xl w-[65%] max-md:w-[100%] gap-2">
                    ...
                    <div class="grid grid-cols-3 p-4">
                        ...
                        <button class="btn-pic col-span-2 relative border w-fit bg-white shadow-lg rounded-xl p-3">
                            <img id="cat_pic1" class=" outline-none w-[100px] h-auto rounded-xl ">
                        </button>   
                    </div>
                    <div class="grid grid-cols-3 p-4">
                        <div>
                        ...
                        <button class="btn-document col-span-2 relative border w-fit bg-white shadow-lg rounded-xl p-3">
                            <img id="cat_document1" class=" outline-none w-[100px] h-auto rounded-xl ">
                        </button>                      
                    </div>
                    <button onClick="SubmitCatEdit()" type="button" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 rounded-lg text-sm px-5 py-2.5 text-center ml-auto mr-5 flex">บันทึก</button>       
            </div>

            <!-- รายละเอียดแมว madal ดูโปรไฟล์แมว -->
            <div id="modal-detail-pic" class="modal hidden fixed z-[100] flex my-auto left-0 top-0 w-screen h-screen overflow-auto" onclick="closeModalpic()">
                <div class="modal-content bg-white m-auto p-[10px] rounded-md drop-shadow-xl w-[30%] max-md:w-[100%] gap-2" onclick="event.stopPropagation()">                    
                    <div class="relative flex items-ceter justify-center">
                        <img id="cat_pic2" src="" class=" w-full h-full object-cover" alt="">
                    </div>
                </div>
            </div>
             <!-- รายละเอียดแมว madal ดูเอกสารแมว-->
            <div id="modal-doucment" class="modal hidden fixed z-[100] flex my-auto left-0 top-0 w-screen h-screen overflow-auto" onclick="closeModal()">
                <div class="modal-content bg-white m-auto p-[10px] rounded-md drop-shadow-xl w-[30%] max-md:w-[100%] gap-2" onclick="event.stopPropagation()">                  
                    <div class="relative flex items-ceter justify-center">
                        <img   id="cat_document2" src="" class=" w-full h-full object-cover" alt="">
                    </div>
                </div>
            </div>
        <!-- END MODAL CONTENT -->
                ...
                <script>
                    $(document).ready(function() {
                        $('.btn-edit-cat').on('click', function() {
                            ...
                            $('#cat_name1').val($(this).data('cat_name'));
                            $('#cat_breed1').val($(this).data('cat_breed'));
                            $('#cat_weight1').val($(this).data('cat_weight'));
                            $('#cat_gender1').val($(this).data('cat_gender'));
                            $('#cat_date1').val($(this).data('cat_date'));
                            $('#cat_pic1').attr('src', $(this).data('cat_pic'));
                            $('#cat_document1').attr('src', $(this).data('cat_document'));
                        ...
                        $('#details-close').on('click', function() {
                         ...
                         $('.btn-cat-delete').on('click', function () {
                            Swal.fire({
                                title: `คุณแน่ใจหรือไม่? <br><b class="text-xl font-medium">(ชื่อแมว: ${$(this).data('cat_name')})</b>`,
                         ...
                         //modaldocumnet ดูโปรไฟล์แมว
                         $('.btn-pic').on('click', function() {
                            $('#cat_id1').val($(this).data('cat_id'));
                            $('#cat_pic2').attr('src', $(this).find('img').attr('src'));
                            $('#modal-detail-pic').removeClass('hidden');
                        });
                        $('#details-close').on('click', function() {
                            $('#modal-doucment').addClass('hidden');
                        });  
                        //modaldocumnet ดูเอกสารแมว
                        $('.btn-document').on('click', function() {
                            $('#cat_document2').attr('src', $(this).find('img').attr('src'));
                            $('#modal-doucment').removeClass('hidden');
                        });
                        $('#details-close').on('click', function() {
                            $('#modal-doucment').addClass('hidden');
                        });  
            ...
            function closeModalpic(){
                document.getElementById("modal-detail-pic").classList.add("hidden")
            }
            function closeModal() {
                document.getElementById("modal-doucment").classList.add("hidden");
            }
            var _image64_cat = '';
            ...
            function fileHanddle_Doc(src){
                ...
                $("#cat_document1").attr("src", src);

                _image64_doc = src;
            }

            function SubmitCatEdit() {
                console.log('value = ' + _image64_doc);
