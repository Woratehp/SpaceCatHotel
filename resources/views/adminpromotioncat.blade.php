...
@section('Content')
    <div class = "  flex"  >
                    ...
                    <div class="flex">
                        <div class="px-4 py-6">
                            <div id="image-preview" class="max-w-sm p-8 mb-4 bg-gray-100 border-dashed border-2 border-gray-400 rounded-lg items-center mx-auto text-center cursor-pointer">
                                <input onchange={fileChosen_Single(event)} id="promotion_shower" type="file" class="hidden" accept="image/*" />
                                <label for="promotion_shower" class="cursor-pointer">
                ...
                @if(sizeof($promotionshower))   
                    <div class="grid grid-cols-3 gap-6 px-[10em] max-md:grid-cols-2 max-sm:grid-cols-1">
                        ...
                        <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow text-center ">
                            <img src="{{ URL('/uploads/'. $promotionshower->promotion_shower) }}"class=" mt-3 mx-auto flex h-[15em] w-[19em]  flex items-center justify-center rounded-lg shadow-lg hover:scale-125 duration-300"/>  
                            <button type="button" data-promotionshower_id="{{$promotionshower->promotionshower_id}}" class="btn-promotion-delete text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 mt-5 ">ลบ</button>
                        </div>                                           
                    @endforeach               
                    </div>
                 @endif
        ...
        <script>
                ...
                $('.btn-promotion-delete').on('click', function () {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            SubmitPromotionshowerDelete($(this).data('promotionshower_id'));
                        }
                    })
                 });
            });
            
            function SubmitPromotionshowerDelete(promotionshower_id) {
                fetch("{{ Route('SubmitPromotionshowerDelete') }}", {
                    method: "POST",
                    headers: {
                    ...
                    body:JSON.stringify(
                        {
                            promotionshower_id: promotionshower_id,
                        }
                    )
                })
                .then(async response => {
            ...
            function Submitpromotioncat() {
                fetch("{{ Route('Submitpromotioncat') }}", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Accept": "application/json",
                    ...
                    body:JSON.stringify(
                        {
                            promotion_shower: document.getElementById("promotion_shower"),
                            imagePromotioncat: _image64_single
                        }
                    )
                })
                .then(async response => {
            ...
            function fileHanddle_Single(src){
                $("#promotionshower").attr("src", src);
                _image64_single = src;
            }

            const uploadInput = document.getElementById('promotion_shower');
